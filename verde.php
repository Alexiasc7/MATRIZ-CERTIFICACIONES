...
echo '<tr>';
echo '<th style="background-color:#64c14a">NÚMERO</th>';
echo '<th style="background-color:#64c14a">NOMBRE</th>';

foreach ($columnasConValor as $columna) {
    if ($columna !== 'nuevo') {
        echo '<th style="background-color:#64c14a; text-transform: uppercase; font-weight: 600;">' . $columna . '</th>';
    }
}

echo '</tr>';

$valorBuscado = "Verde"; // Reemplazar
...
while ($filaRegistros = @mysqli_fetch_assoc($resultadoRegistros)) {
    echo '<tr style="background-color:#64c14a8f">';
    ...
    foreach ($columnasConValor as $columna) {
        if ($columna !== 'nuevo') {
            if ($filaRegistros[$columna] == $valorBuscado) {
                echo '<td><img src="../PROJECT/assets/imgs/verde.png"></td>';
            } else {
                echo '<td></td>';
            }
        }
