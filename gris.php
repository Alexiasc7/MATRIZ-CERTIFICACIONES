...
echo '<tr>';
echo '<th style="background-color:#6c757d">NÚMERO</th>';
echo '<th style="background-color:#6c757d">NOMBRE</th>';

foreach ($columnasConValor as $columna) {
    if ($columna !== 'nuevo') {
        echo '<th style="background-color:#6c757d; text-transform: uppercase; font-weight: 600;">' . $columna . '</th>';
    }
}

echo '</tr>';

$valorBuscado = "Gris"; // Reemplazar
...
while (@$filaRegistros = mysqli_fetch_assoc($resultadoRegistros)) {
    echo '<tr style="background-color:#57565673; text-transform: uppercase;">';
    ...
    foreach ($columnasConValor as $columna) {
        if ($columna !== 'nuevo') {
            if ($filaRegistros[$columna] == $valorBuscado) {
                echo '<td><img src="../PROJECT/assets/imgs/gris.png"></td>';
            } else {
                echo '<td></td>';
            }
        }
