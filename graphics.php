...
echo '<tr>';
echo '<th style="background-color:#fc5551">NÚMERO</th>';
echo '<th style="background-color:#fc5551">NOMBRE</th>';

foreach ($columnasConValor as $columna) {
    if ($columna !== 'nuevo') {
        echo '<th style="background-color:#fc5551; text-transform: uppercase; font-weight: 600;">' . $columna . '</th>';
    }
}

echo '</tr>';

$valorBuscado = "Rojo"; // Reemplaza por cada valor buscado
...
while (@$filaRegistros = mysqli_fetch_assoc($resultadoRegistros)) {
    echo '<tr style="background-color:#c14a4a8f">';
    ...
    foreach ($columnasConValor as $columna) {
        if ($columna !== 'nuevo') {
            if ($filaRegistros[$columna] == $valorBuscado) {
                echo '<td><img src="../PROJECT/assets/imgs/rojo.png"></td>';
            } else {
                echo '<td></td>';
            }
        }
