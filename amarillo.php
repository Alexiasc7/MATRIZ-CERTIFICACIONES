...
echo '<tr>';
echo '<th style="background-color:#fceb51">NÚMERO</th>';
echo '<th style="background-color:#fceb51">NOMBRE</th>';

foreach ($columnasConValor as $columna) {
    if ($columna !== 'nuevo') {
        echo '<th style="background-color:#fceb51; text-transform: uppercase; font-weight: 600;">' . $columna . '</th>';
    }
}

echo '</tr>';

$valorBuscado = "Amarillo"; // Reemplaza "Amarillo" 
...
while (@$filaRegistros = mysqli_fetch_assoc($resultadoRegistros)) {
    echo '<tr style="background-color:#fdef608f; text-transform: uppercase;">';
    ...
    foreach ($columnasConValor as $columna) {
        if ($columna !== 'nuevo') {
            if ($filaRegistros[$columna] == $valorBuscado) {
                echo '<td><img src="../PROJECT/assets/imgs/amarillo.png"></td>';
            } else {
                echo '<td></td>';
            }
        }
